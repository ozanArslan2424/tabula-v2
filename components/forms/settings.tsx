...
export const SettingsForm = ({ user }: { user: UserType }) => {
  ...
  return (
    <form className="w-full space-y-4" onSubmit={handleSubmit(onSubmit)}>
      {success && !error && <Message variant="success">Settings saved.</Message>}
      {!success && error && <Message variant="error">There was an error. Please try again.</Message>}
      {errors.root && <Message variant="error">{errors.root.message}</Message>}
      <Label>
        <span>Email</span>
        <Input {...register("email")} type="email" disabled={isSubmitting} required />
      ...
      <Label>
        ...
        <PasswordInput {...register("newPassword")} placeholder="********" disabled={isSubmitting} required />
        {errors.newPassword && <Message variant="formerror">{errors.newPassword.message}</Message>}
      </Label>
