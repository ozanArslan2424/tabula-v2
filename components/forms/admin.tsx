...
import { createNewToken, createUser, deleteUser, updateUserRole } from "@/lib/actions/admin.actions";
...
export function InviteForm() {
  ...
  return (
    <form className="w-full space-y-4" onSubmit={handleSubmit(onSubmit)}>
      ...
      {success && !error && <Message variant="success">Invite sent!</Message>}
...
export function DeleteUser({ users }: { users: UserTableType[] }) {
  ...
  return (
    <form className="w-full space-y-4" onSubmit={handleSubmit}>
      ...
      {success && !error && <Message variant="success">User deleted!</Message>}
      {!success && error && <Message variant="error">There was an error. Please try again.</Message>}
    </form>
  );
}
...
export function ChangeUserRole({ users }: { users: UserTableType[] }) {
  ...
  return (
    <form className="w-full space-y-4" onSubmit={handleSubmit}>
      ...
      {success && !error && <Message variant="success">User role changed!</Message>}
      {!success && error && <Message variant="error">There was an error. Please try again.</Message>}
    </form>
  );
}
