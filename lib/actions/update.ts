...
// updateBookSettings
export const updateBookSettings = async (values: z.infer<typeof BookSettingsSchema>) => {
...
export const updateNote = async (noteId: string, editorContent: string) => {
  try {
    await db.update(noteTable).set({ content: editorContent }).where(eq(noteTable.id, noteId));
...
export async function updateNoteTitle(noteId: string, title: string) {
  await db.update(noteTable).set({ title: title }).where(eq(noteTable.id, noteId));
}

// updateTask
export const updateTask = async (taskId: string, completed: boolean) => {
  try {
    await db.update(taskTable).set({ completed: completed }).where(eq(taskTable.id, taskId));
